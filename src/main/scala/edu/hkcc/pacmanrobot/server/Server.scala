...
object Server extends App {
  ...
  val MovementCommandThread = new Thread(new Runnable {
    override def run(): Unit = {
      ...
      while (true) {
        movementCommandMessengers += new MovementCommandMessenger(serverSocket.accept(), Config.PORT_MOVEMENT_COMMAND) {
          override def autoGet(message: MovementCommand): Unit = {
            val robotId = controllerRobotPairManager.getRobotId(getRemoteMacAddress)
  ...
  def switchGameStatus(gameStatus: GameStatus) = {
