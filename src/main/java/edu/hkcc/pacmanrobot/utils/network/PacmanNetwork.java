        ...
        public void run() {
            while (!isInterrupted()) {
                try {
                    ...
                    DatagramSocket socket = new DatagramSocket(PacmanNetwork.PORT);
                    byte[] buffer = PacmanNetwork.MESSAGE_SERVER.getBytes();
                    while (!isInterrupted()) {
                        InetAddress[] inetAddresses = InetAddress.getAllByName(PacmanNetwork.GROUP_NAME);

                        for (InetAddress inetAddress : inetAddresses) {
                            DatagramPacket packet = new DatagramPacket(buffer, buffer.length, inetAddress, PacmanNetwork.PORT);
                            try {
                                socket.send(packet);
                            } catch (IOException e) {
                                e.printStackTrace();
                        ...
                        try {
                            Thread.sleep(PacmanNetwork.INTERVAL);
